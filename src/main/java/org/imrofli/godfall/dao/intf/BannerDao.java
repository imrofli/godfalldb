...
public interface BannerDao extends JpaRepository<Banner, Long> {
    ...
    @Query(value = "SELECT b from Banner b LEFT join fetch b.elements LEFT join fetch b.affinities left JOIN FETCH b.traitSlots where b.displayName like :name order by b.name")
    Set<Banner> findAllAndFetchElementsAndAffinitiesByNameOrderByName(@Param("name") String name);
