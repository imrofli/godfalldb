...
public interface LifeStoneDao extends JpaRepository<LifeStone, Long> {
    ...
    @Query(value = "SELECT a from LifeStone a LEFT join fetch a.elements LEFT join fetch a.affinities left JOIN FETCH a.traitSlots where a.displayName like :name order by a.name")
    Set<LifeStone> findAllAndFetchElementsAndAffinitiesByNameOrderByName(@Param("name") String name);
