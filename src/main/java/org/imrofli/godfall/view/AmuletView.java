...
@Route(value = "amulets", layout = MainView.class)
public class AmuletView extends HorizontalLayout {
    private static final Logger LOGGER = LoggerFactory.getLogger(AmuletView.class);
    ...
    private void handleRowSelectTrinket(SelectionEvent<Grid<Trinket>, Trinket> gridTrinketSelectionEvent) {
        if(gridTrinketSelectionEvent!=null){
            if(gridTrinketSelectionEvent.getFirstSelectedItem().isPresent()){
                ...
                LOGGER.info("Getting Amulet: {}", gridTrinketSelectionEvent.getFirstSelectedItem().get().getName());
    ...
    @PostConstruct
    public void init() {
        trinketGrid.setItems(trinketService.getAmulets());
