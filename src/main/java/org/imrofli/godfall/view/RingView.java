...
@Route(value = "rings", layout = MainView.class)
public class RingView extends HorizontalLayout {
    private static final Logger LOGGER = LoggerFactory.getLogger(RingView.class);
    ...
    private void handleRowSelectTrinket(SelectionEvent<Grid<Trinket>, Trinket> gridTrinketSelectionEvent) {
        if(gridTrinketSelectionEvent!=null){
            if(gridTrinketSelectionEvent.getFirstSelectedItem().isPresent()){
                ...
                LOGGER.info("Getting Ring: {}", gridTrinketSelectionEvent.getFirstSelectedItem().get().getName());
    ...
    @PostConstruct
    public void init() {
        trinketGrid.setItems(trinketService.getRings());
